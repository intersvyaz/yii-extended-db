...
class DbCommand extends \CDbCommand
{
    ...
    {
        /**
         * Если вдруг в запросе есть специальные комментарии, а параметры пришли пустые, то оставляем запрос как есть.
         * Если это сильно сложный запрос какого-то списка, то там либо будет обязательно какие-то условия, либо же
         * будет анализатор, что никаких условий не выставлено и тогда запрос совсем не будет выполнять.
         * Если же это простой запрос, то лучше не тратить лишнее время на разбор комментариев, а скормить базе как есть.
         */
        if (empty($params)) {
            return $query;
        }
        ...
        /**
         * Многоитерационный разбор однострочных комментариев. Тут важный момент - берем вместе с переводом строки,
         * либо это будет "строка" до конца всей многострочной строки (в случае, если это последняя строка, там нет
         * перевода строки). В противном случае, если мы будем заменять без перевода строки вот такой текст SQL:
         * ...
         * --*paramName
         * --*paramName AND t.name = :paramName
         * ...
         * то первое совпадение нам дает искомую строку "--*paramName", которая во всем тексте заменяется на "", и тем
         * самым мы раскомментируем тот текст, который не должен был быть раскомментирован, если такого параметра не
         * было выставлено.
         * WhiteSpace (\s) не используем, т.к. это может быть и переводом строки, а его нам захватывать не нужно.
         * Табуляции в sql запросах не приняты, хотя некоторые особо одаренные личности их все таки используют, поэтому
         * их обрабатываем тоже.
         */
        while (true) {
            if (preg_match_all('#--\*([\w|]+)((( |\t).*)?(\R|$))#', $query, $matches)) {
                $count = count($matches[0]);
                for ($i = 0; $i < $count; $i++) {
                    $query = $this->replaceComment($query, $matches[0][$i], $matches[2][$i], $matches[1][$i], $params);
                }
         ...
         * Те, которые были внутри комментариев, были разобраны при обработке комментария.
         */
